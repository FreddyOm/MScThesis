...
effects, games with higher triangle density, and new lighting technolog. One example is 
\emph{cube mapping}, which was introduced to the pipeline with the release of the 
\emph{GeForce 256} and enabled real-time reflections shown in  Figure 
...
for computing three-dimensional data. These new innovations increased the programmability 
and customization capabilities of the pipeline, and are consequently following a trend that began decades ago.
...
Considering the possibilities created by the advances in \ac{GPUDR}, this work aims to implement 
effecive \ac{GPU}-driven occlusion culling for voxel-based rasterized rendering. It proposes 
a possible approach to integrate a screen-space occlusion culling algorithm using large occluders 
into a voxel rendering pipeline, while not affect the real-time interactivity of the application. \\
