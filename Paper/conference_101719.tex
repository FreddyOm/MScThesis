...
%Using the \ac{PONOC} configuration, 
%each octree node is tested for occlusion in parallel on the \ac{GPU}. This results in less 
%screen-space occlusion tests compared to \ac{PMOC} but also leads to higher voxel counts after the 
%occlusion culling.\\
...
\begin{figure}
    \begin{center}
        ...
        \begin{axis}
            ...
            \legend{No Culling, \ac{PONOC}, \ac{PMOC}}
        \end{axis}
        \end{tikzpicture}
    \end{center}
...
\begin{figure}
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    \caption{The depth prepass timings measured on the \ac{GPU}. The dashed lines refer to the \ac{PONOC} 
    configuration and the solid lines refer to the \ac{PMOC} configuration.}
...
Our approach can be used to increase performance by up to $10000\%$ for suitable scenes while preserving a 
manageable overhead. In addition, it can be used for dynamically changing scenes since the best occluders 
can be determined during runtime, instead of being manually picked beforehand. \\
