...
amount of voxels per node. Figure \ref{fig:} shows that the voxels are grouped into octree nodes. 


\begin{figure}[h]
    \centering
    \includegraphics[width=100px]{images/graphics/lucy-voxel-octree-viz.png}
    \includegraphics[width=300px]{images/graphics/lucy-voxel-octree-viz-2.png}
    \caption{The input triangle mesh (left) and the voxelized and parsed voxel mesh (right). 
    Model \emph{Lucy} from \emph{The Stanford 3D Scanning Repository} \cite{Stanford23}.}
    \label{fig:trimesh-to-voxel-mesh}
\end{figure}
