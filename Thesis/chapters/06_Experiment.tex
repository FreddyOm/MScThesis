...
per-meshlet occlusion culling. Although the per-meshlet culling connfiguration still accelerated the \ac{GPU} 
...
\begin{figure}[!htb]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      bar width=8pt,
      xlabel={Time ($\mu$s)},
      symbolic y coords={HairballOOC, HairballMOC},
      yticklabels={B, A},
      ytick=data,
      legend style={at={(0.5,1.05)}, anchor=south, legend columns=-1}
    ]

    \addplot [fill=green!80]  table [col sep=comma, x=DepthPrepass, y=Label]  {plotdata/gpu/hairball_gpu_profiling_frame.csv};
    \addplot [fill=blue!60]   table [col sep=comma, x=DispatchMesh, y=Label]  {plotdata/gpu/hairball_gpu_profiling_frame.csv};
    \addplot [fill=orange!60] table [col sep=comma, x=Misc,         y=Label]  {plotdata/gpu/hairball_gpu_profiling_frame.csv};
  ...
  \begin{tikzpicture}
    ...
    \addplot [fill=red!80]    table [col sep=comma, x=DispatchBestOccluders,  y=Label]  {plotdata/gpu/hairball_gpu_profiling_prepass.csv};
    \addplot [fill=pink!60]   table [col sep=comma, x=CopyTex,                y=Label]  {plotdata/gpu/hairball_gpu_profiling_prepass.csv};
    \addplot [fill=yellow!60] table [col sep=comma, x=GenMipChain,            y=Label]  {plotdata/gpu/hairball_gpu_profiling_prepass.csv};
...
\noindent
The measurements showed that the culling significantly benefitted the \ac{GPU} performance throughout all 
models. Figure \ref{fig:gpu-performance-overview} shows and overview of all models comparing the time spent 
on the \ac{GPU} between both tested configurations. The \ac{CPU} performance did not show any significant 
differences that could reliably traced back to the tested approaches. Small differenes were usually in the 
order of about 1-10 microseconds and could not be reliably attributed to either of the two culling 
configurations. \\

[@TODO: Add overdraw images and throughput data]
