...
\cite{AkenineMoeller2018}. In this section, the most common culling techniques are presented and the ones considered
for the outlined approach are specifically highlighted and analyzed. 
...
\Fn{\FMain{float4 basePosAndScale}}
{
    ...
    float radius $\gets$ abs(basePosAndScale.w)\;
...
\cite{AkenineMoeller2018}. It is a fundamental basis on which the outlined approach relies on, which will be 
...
to adopt similar techniques \cite{Karis2021}. This is the algorithm used in the proposed implementation. It shares its basic 
...
\noindent
The \ac{TPOC} algorithm is used with minimal changes to the selection of best occluders. Depth Reprojection could 
in general be used as well, but is not within the scope of this work and therefore is left to be evaluated by future 
work.
...
There are several other advantages over the traditional Vertex Shaded Pipeline, which will not be listed to the full 
...
\noindent
The approach discussed in this work uses view frustum culling of individual meshlets to further optimize performance. 
A simple implementation was already included in the framework used for the given implementation.
