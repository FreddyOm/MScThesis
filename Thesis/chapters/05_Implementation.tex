...
\subsubsection*{HiZ Generation} \label{subsec-highz-generation}

Now that the depth buffer has data from possible occluders, the \ac{HiZ} generation is started. 
The z-pyramid is generated by a compute shader, which samples four texels of the input texture 
...
\cite{binvox} is used. It is a command line tool that converts triangle meshes (e.g. \emph{.obj}) to a volumetric 
voxel representation. The resulting file can be consumed by the engine. Figure \ref{fig:trimesh-to-voxel-mesh} shows 
...
\subsection*{Culling and Draw Call} \label{subsec-culling-and-draw-call}
