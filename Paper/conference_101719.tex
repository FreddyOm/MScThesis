...
The primary experimental setup tested \ac{TPOC-PM} and compared it to \ac{TPOC-PON}, which uses whole 
octree nodes instead of individual meshlets for the culling. \\
%Using the \ac{TPOC-PON} configuration, 
%each octree node is tested for occlusion in parallel on the \ac{GPU}. This results in less 
%screen-space occlusion tests compared to \ac{TPOC-PM} but also leads to higher voxel counts after the 
%occlusion culling.\\
