...
\begin{figure}[h!]              % Lucy renderTimes
  \begin{center}
    \begin{tikzpicture}
      \begin{axis}[
          width=\linewidth,
...
\noindent
The overall framerate of the application is artificially clamped to 30, 60 or 120 \emph{Hz} by \emph{Diligent Engine}, 
...
more reliable results. Figure \ref{plt:lucy-256-culling-cpu-time} shows the isolated computation time of the 
\ac{CPU} \emph{Render()} function. This is a good indicator of how the computation time changes and it includes the 
\ac{GPU} work that is called during the function, like \emph{DispatchMesh()} during the depth prepass and the final 
draw call. \\

\noindent [@TODO: Fix ref to double plot]
Figure \ref{plt:lucy-256-culling-cpu-time} shows the time the update function took to calculate the new camera 
position. This function was expected to be static in runtime, regardless of the culling configuration. The graphs 
show that this is generally correct, as both are not significantly diverging.\\
...
completely happens on the \ac{GPU} end of the piepeline. The same was expected for all tested models. 
...
culling algorithm intended, the time spent drawing the meshes could be reduced by $\%$ on average.
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      yticklabels={B, A},
      ytick=data,
      legend style={at={(0.5,1.5)}, anchor=south, legend columns=-1}
    ]
...
In figure \ref{plt:bunny-256-culling-render-time} there are a small amount of frames between frames 280 and 380, 
which seem to have been faster in execution using the per-meshlet occlusion culling, however, it is not obvious 
why there would be any difference here. Similarly, figure \ref{plt:bunny-256-culling-cpu-time} also shows 
small differences that are not considered significant enough to make any assumptions about them. Therefore, for 
this particular model the \ac{CPU} compute times are also considered to be static between both configurations.
...
\begin{figure}[h!]              % Torus renderTimes
  \begin{center}
    \begin{tikzpicture}
      \begin{axis}[
          width=\linewidth,
