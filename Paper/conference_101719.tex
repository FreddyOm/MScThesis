...
\author{
    ...
    \else
        \IEEEauthorblockN{1\textsuperscript{st} Author}
        \IEEEauthorblockA{\textit{Department of Computer Science} \\
        \textit{Media University Stuttgart}\\
        Stuttgart, Germany}
        *Corresponding author
...
\begin{abstract}
    ...
    available in its original form for dynamic voxel-based scenes. Our TPOC implementation uses a 
    sparse voxel octree (SVO) to efficiently schedule voxel data with respect to their spatial 
    characteristics and replaces full octree nodes with larger geometry for fast computation of the 
    HZB. This not only allows the application of HZB on a dynamic voxel scene, but also results in 
    increased performance of up to 90\% compared to a rendering pipeline without occlusion culling. 
    Furthermore, we show that the use of the mesh shading pipeline can adapt the HZB to efficiently 
    occlude a significant number of voxels, resulting in an overall performance increase of up to 
    42\% compared to a per-octree-node culling approach.
\end{abstract}
...
and global illumination \cite{b18}. However, the rasterized approach to voxel rendering has 
not been very present in the entertainment industry, although the rasterized rendering 
pipeline has been upgraded significantly over the last decade. A central overhaul is the 
mesh shading pipeline, which has been introduced to \ac{GPU}s in 2018 \cite{b17}. It features 
a new way of preprocessing vertices using a more general compute architecture that allows 
for more efficient \ac{GPU} computations \cite{b11}. \\
...
for the recent increase in geometrical density \cite{b12, b13}. The approach is heavily reliant 
on the presence of large, static occluder geometry, which makes it inefficient to be used in 
voxelized scenes in the form it has been so far. \\
...
performance increase. In this context, we focus on scenes that not only approximate the 
surface by using voxels but also include a voxel-based volume that might be exposed or altered 
during runtime. A great amount of the algorithm relies on \ac{GPU}-driven computations, which 
allow for millions of voxels to be drawn efficiently. \\
...
the nonvisible volume within a voxel scene and uses it as an occluder for \ac{GPU}-based \ac{HZB}. 
...
far objects, resulting in an efficient way to reject invisible meshes. In their work, they tested 
their algorithm using ray tracing on a very high resolution scene. Using \ac{HZB}, ``the entire
process took 6.45 seconds. Rendering this model using traditional Z buffering [...] took 
approximately one hour and fifteen minutes'' \cite{b1}. \ac{HZB} is the fundamental basis of the 
\ac{TPOC} algorithm we present here. \\
...
of the best occluders is thus not possible as it is for static, nonvoxel triangle meshes. \\
...
the camera than the minimum \emph{z} value, the voxel can safely be assumed to be occluded. The 
correct mip map level for sampling is calculated so that the bounding box of the voxel is best 
approximated by one texel. The complete rendering loop is shown in Fig.~\ref{fig:rendering-loop}.
...
\begin{table}[htbp]
    \begin{center}
        ...
        \end{tabular}
    \end{center}
    \caption{Measurements of best occluder query. Lower is better.}
...
various camera angles. The table indicates an overall better culling performance when using \ac{TPOC-PM} as 
compared to the \ac{TPOC-PON} configuration. The \ac{TPOC-PM} configuration culled an average of $12.2 \%$ 
more voxels and always resulted in fewer voxels computed by the rasterizer and the pixel shader. The 
measurements also suggest that larger scenes with more voxels lead to a higher culling efficiency. This seems 
plausible, as a higher voxel count tends to result in a higher number of best occluders when using a static 
maximum number of voxels per octree node. As the \emph{Hairball} scene shows, our \ac{TPOC} implementation 
works in favor of large, dense geometry and is challenged by smaller details, large empty spaces, and holes.
...
table~\ref{tab:cpu-performance}. It was measured over multiple frames for each culling configuration 
to evaluate the difference in runtime performance for the render thread functions of the application. 
The table shows that the average \ac{CPU} performance does not change significantly between the 
different culling configurations. The small changes that are visible in measurements are attributed 
to the \ac{GPU} performance differences presented below. Because of the \ac{GPU}-driven nature of 
\ac{TPOC}, which results in a low memory bandwidth between \ac{CPU} and \ac{GPU}, these changes are 
expected to be small. Most of the work is executed on the \ac{GPU} and the \ac{CPU} implementations 
do not change considerably between the \ac{TPOC-PON} and the \ac{TPOC-PM} configuration. \\
...
\begin{table}[htbp]
    ...
    \end{tabular}
\end{center}
\caption{Average \ac{CPU} performance using different occlusion culling configurations. Lower is better.}
...
load on those pipeline stages, which in turn reduces the overall render time. The data show that the performance 
gains outweigh the overhead introduced by the occlusion culling in all tested cases.
...
pixels using no occlusion culling, the \ac{TPOC-PON}, and the \ac{TPOC-PM} configuration. 
Fig.~\ref{fig:overdraw} shows an exemplary overdraw visualization for the \emph{Bunny} scene. \\
...
in all cases the \ac{TPOC-PM} configuration was able to reduce overdraw by up to $84 \%$ compared with 
a pipeline without culling and by up to $47 \%$ compared with \ac{TPOC-PON}.
...
the scenes invisible voxel data and subsequently uses the data to efficiently draw the best occluders for occlusion 
culling. \\
...
and \emph{Stanford Bunny}, Ketra Games for \emph{Terrain}, and Patrick Min for \emph{binvox}.
...
\bibitem{b18} NVIDIA, ``NVIDIA ADA GPU ARCHITECTURE'', nvidia.com, https://images.nvidia.com/aem-dam/Solutions/geforce/ada/nvidia-ada-gpu-architecture.pdf, 2023 [online] (accessed Feb. 19, 2025)
\bibitem{b17} C. Kubisch, ``Introduction to Turing Mesh Shaders,'' nvidia.com, https://developer.nvidia.com/blog/introduction-turing-mesh-shaders/, 2018 [online] (accessed Feb. 19, 2025)
...
\bibitem{b12} B. Karis, R. Stubbe, and G. Wihlidal, ``A Deep Dive into Nanite Virtualized Geometry,'' realtimerendering.com, in ACM SIGGRAPH 2021 Courses, Advances in Real-Time Rendering in Games, Part 1, https://advances.realtimerendering.com/s2021/index.html [online] (accessed Feb. 17, 2025)
\bibitem{b13} Remedy Entertainment, ``How Northlight makes Alan Wake 2 shine,'' remedygames.com, https://www.remedygames.com/article/how-northlight-makes-alan-wake-2-shine [online] (accessed Feb. 17, 2025)
...
\bibitem{b7} Stanford Computer Graphics Laboratory, ``The Stanford 3D Scanning Repository,'' stanford.edu, https://graphics.stanford.edu/data/3Dscanrep/, 2023 [online] (accessed Feb. 17, 2025)
\bibitem{b15} Ketra Games, ``Creating Terrain from Heightmaps - Unity Game Development Tutorial,'' kreta-games.com, https://www.ketra-games.com/2021/08/creating-terrain-from-heightmaps-unity-game-tutorial.html, 2021 [online] (accessed Feb. 17, 2025)
\bibitem{b8} S.Laine and T. Karras, ``Two Methods for Fast Ray-Cast Ambient Occlusion,'' in Eurographics Symposium on Rendering, 2010
\bibitem{b16} Sketchfab, ``Hairball,'' sketchfab.com,  https://sketchfab.com/3d-models/hairball-db894e830ad94216ae6188396d707aff, 2011 [online] (accessed Feb. 17, 2025)
\end{thebibliography}
\vspace{12pt}
\end{document}
